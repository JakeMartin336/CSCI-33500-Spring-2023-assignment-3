...
int NextPrime_Linear(size_t n) 
{
  if (n % 2 == 0)
    ++n;  
  while (!IsPrime_Linear(n)) n += 2;  
    return n;
}
}  // namespace
...
class HashTableLinear {
 public:
  ...
  explicit HashTableLinear(size_t size = 101) : array_(NextPrime_Linear(size))
    { MakeEmpty(); }
 ...
 private:        
  ...
  void Rehash() 
  {
    ...
    array_.resize(NextPrime_Linear(2 * old_array.size()));
    for (auto & entry : array_)
      entry.info_ = EMPTY;
